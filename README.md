...
| `theme_files/` | Канонические стартовые документы для MinIO и SQL source manifest |
...
`alembic_version`. После миграций backend синхронизирует
`backend/src/core/rag/source_manifest.json` из репозиторного `theme_files/`:
на чистой базе создаются/обновляются учебные темы, связи с файлами и актуальные
эталонные вопросы/ответы, а существующие проиндексированные файлы не
сбрасываются обратно в `uploaded`.
...
Если нужно вручную пересинхронизировать стартовые документы из репозитория и
затем пересобрать Qdrant:
