...
test('full manual flow and screenshots', async ({ browser }, testInfo) => {
  const screenshotPath = (filename) => testInfo.outputPath('screenshots', filename)
  ...
  await studentPage.screenshot({ path: screenshotPath('1_student_register.png') })
  ...
  await studentPage.screenshot({ path: screenshotPath('2_student_register_filled.png') })
  await studentPage.getByRole('button', { name: 'Создать аккаунт' }).click()

  // Dashboard
  await expect(studentPage).toHaveURL(/\/app$/)
  await studentPage.screenshot({ path: screenshotPath('3_student_dashboard.png') })

  // Course Page
  await studentPage.goto(appUrl(STUDENT_BASE_URL, '/app/course'))
  await studentPage.screenshot({ path: screenshotPath('4_student_course.png') })

  // Exams Page
  await studentPage.goto(appUrl(STUDENT_BASE_URL, '/app/exams'))
  await studentPage.screenshot({ path: screenshotPath('5_student_exams.png') })
  ...
  await studentPage.screenshot({ path: screenshotPath('6_student_exam_session.png') })

  // Answer question
  await expect(studentPage.getByText('Вопрос')).toBeVisible()
  await studentPage.getByLabel('Ваш ответ').fill('42')
  await studentPage.screenshot({ path: screenshotPath('7_student_exam_answered.png') })
  await studentPage.getByRole('button', { name: 'Отправить ответ' }).click()

  // Stats Page
  await studentPage.goto(appUrl(STUDENT_BASE_URL, '/app/stats'))
  await studentPage.screenshot({ path: screenshotPath('8_student_stats.png') })

  // Profile Page
  await studentPage.goto(appUrl(STUDENT_BASE_URL, '/app/profile'))
  await studentPage.screenshot({ path: screenshotPath('9_student_profile.png') })
  ...
  await adminPage.screenshot({ path: screenshotPath('10_admin_login.png') })

  // Login
  await adminPage.getByLabel('Email').fill('admin@example.com')
  await adminPage.getByLabel('Пароль').fill('Admin123!')
  await adminPage.screenshot({ path: screenshotPath('11_admin_login_filled.png') })
  await adminPage.getByRole('button', { name: 'Войти' }).click()

  // Dashboard
  await expect(adminPage).toHaveURL(/\/dashboard$/)
  await adminPage.screenshot({ path: screenshotPath('12_admin_dashboard.png') })

  // Knowledge base / Documents
  await adminPage.goto(appUrl(ADMIN_BASE_URL, '/knowledge-base'))
  await adminPage.screenshot({ path: screenshotPath('13_admin_knowledge_base.png') })

  // Questions management
  await adminPage.goto(appUrl(ADMIN_BASE_URL, '/questions'))
  await adminPage.screenshot({ path: screenshotPath('14_admin_questions.png') })
